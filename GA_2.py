...
#        Lamda.append(pf[1][i])
    for i in range(P // 2): #mate
        cind = 0
        Lamda += mate(pf[1][cind],pf[1][cind+1])
        cind += 2
    while len(Lamda) < S: #add new chanllengers
        x = rd.uniform(-20,20)
        Lamda.append(x)
    for i in range(S): #calculate pi for every x in Lamda
        pi = CoFun(Lamda[i])
        new_pi.append(pi)
    pf = pd.DataFrame([new_pi,Lamda])
    pf = pd.DataFrame(pf.values.T)
...
fig.tight_layout()
plt.savefig('MATSCI_C286_Project_1_Q2c.pdf')
plt.show()
